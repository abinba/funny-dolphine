...
async def login(
    ...
    if not await LoginMethodRepo.exists(
    ...
    test_password = bcrypt.hashpw(password=login_password.encode('utf-8'), salt=salt_db.salt.encode('utf-8'))
...
async def register(
    ...
    if await LoginMethodRepo.exists(
            session, login_email=login_email
    ):
        raise HTTPException(
            status_code=409,
    ...
    account = await AccountRepoWithoutId.create(session, username=username, is_active=True)

    await SaltRepo.create(session, salt=salt.decode('utf-8'), account_id=account.account_id)

    account_login_method = await LoginMethodRepo.create(session, account_id=account.account_id,
